...
CStudyDlg1::CStudyDlg1(CIni *pIni ,CWnd* pParent /*=NULL*/ )
	: CDialogEx(IDD_STUDY1, pParent),
	m_pIni(pIni)
{
	g_logger->Log(ILogger::LogLevel::LL_INFO, _T("打开设置界面"));
	m_tabPage4.m_pCini = pIni;
}
...
BEGIN_MESSAGE_MAP(CStudyDlg1, CDialogEx)
	...
	ON_BN_CLICKED(IDOK, &CStudyDlg1::OnBnClickedOk)
END_MESSAGE_MAP()
...
BOOL CStudyDlg1::OnInitDialog()
{
	...
	m_studyTab.InsertItem(2, _T("通信设置"));
	m_studyTab.InsertItem(3, _T("图像设置"));
	...
	//更新配置文件数据到tab页面
	m_tabPage0.updateMember(m_pIni);
	m_tabPage3.updateMember(m_pIni);
...
void CStudyDlg1::OnBnClickedOk()
{
	// TODO: 在此添加控件通知处理程序代码
	//写入配置文件

	m_tabPage0.UpdateData(TRUE);
	int a = m_pIni->WriteInt(_T("Camera Configuration"), _T("ImageHeight"),m_tabPage0.m_iImgeWidth);
	a = m_pIni->WriteInt(_T("Camera Configuration"), _T("LineRate"), m_tabPage0.m_iLineRate);
	a = m_pIni->WriteInt(_T("Camera Configuration"), _T("ExposureTime"), m_tabPage0.m_iExposureTime);
	...
	g_logger->Log(ILogger::LogLevel::LL_INFO, _T("保存设置参数"));
	CDialogEx::OnOK();
}
